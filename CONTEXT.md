...
## Chat
The user-facing name for a Channel and its Transcript. Use reply for a relationship to one message. Do not call the flat Chat a thread. Expose Session only in advanced settings or diagnostics.
...
## Isolated
Config mode. Talk sets the vendor home env (`CODEX_HOME`, `CLAUDE_CONFIG_DIR`, `GROK_HOME`, `KIMI_CODE_HOME`) to Harness Home on the Computer. Unix ~ is This Bot's directory. Isolated login-shell snapshot is off, so Isolated `ls ~` is This Bot's directory, not the host login home. Isolated Seatbelt is off so Isolated can `docker exec` into Screen. The Bot does not load the user's global AGENTS.md, hooks, plugins, MCP, or the host `$HOME/.agents`. Auth is still the host login. Computer default. Override per Bot. Isolated is not "the user asked for Docker." The Session is told it is Isolated. Isolated commands run in Screen. Isolated native host shell stays until Isolated exec.

## Host
Config mode. Unset those env vars. Unix ~ is the host home. The Harness uses the user's real `~/.codex` (and friends). The Session is told it is Host. Host commands run on the host OS. `OPENBOT.md` is not injected. Workspace AGENTS.md files still load because they live in cwd.

## Isolated exec
Post-v1. Talk runs Isolated commands in Screen. Isolated native host shell is off. The Screen-command instruction leaves `OPENBOT.md`. Issue #69.
...
## OPENBOT.md
Locked product file in `shared/`. Isolated user-level instructions. Isolated Sessions are told they are Isolated and must run commands in Screen. Users do not edit it. User-facing editors are All Bots and This Bot only.
