...
The Computer preview is watch-only. Zooming or opening it (the existing fullscreen Computer / >>) is already write. No Takeover button. While zoomed, that Bot's desktop control pauses (SIGSTOP on the host ACP child) so two pointers do not fight. Closing zoom returns to watch-only and SIGCONT.
