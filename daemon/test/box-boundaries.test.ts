...
function delayedFailingBotStore(): NonNullable<BoxOptions["botStore"]> {
  return {
    recoverBotProvisionings: async () => undefined,
    reattachDisplays: async () => undefined,
    stopScreenAttachments: async () => undefined,
    listScreenRuntimes: () => [],
    close: () => undefined,
    send: async () => {
...
test("a fully received request can return a bounded handler failure after the idle interval", async () => {
  const delayedStore = delayedFailingBotStore();
  const limits = {
    ...TEST_TRANSPORT_LIMITS,
    headersTimeoutMs: 100,
